...
- P42 已于 2026-08-05 发布：Worker 与旧 Pages Functions 已移除硬编码备用 Key 和 URL Key，管理读取认证前置于 D1；静态／API 增加基础安全头，API 与后台禁缓存，未知 API、非法询盘子路径和错误方法分别返回 404／405。站点负责人已通过 Wrangler 交互式流程设置 `ADMIN_KEY`，只核对变量名、未读取值。Cloudflare 版本为 `3c3bac5d-ee82-4af6-8cfe-77f92fcea5a7`；生产固定 7 个 URL、安全头、API 状态、sitemap 43/43 与 F29 五图验证通过，未提交询盘或写入 D1。详见 `docs/audits/p42-security-headers-api-exposure.md`。
- P43 已于 2026-08-05 发布：后台管理 Key 改为仅当前标签页会话保存并主动清除旧版持久值；分析 POST 限定可信 Origin、JSON 对象、16 KB 请求体及每 IP 每小时 120 次，D1 新增 `(ip, created_at)` 查询索引。Cloudflare 版本为 `3b456207-4c77-4b86-bc22-ae9d6654262f`；生产固定 URL、后台会话脚本与安全头、sitemap 43/43、SY300/F29 身份及 F29 五图验证通过，未发送生产分析／询盘 POST 或主动写 D1。详见 `docs/audits/p43-admin-session-analytics-abuse-protection.md`。
- P44 已于 2026-08-05 发布：JSON API 取消默认通配 CORS，新闻与询盘公开 POST 保持开放；分析与后台管理请求仅对生产站点及本地开发 Origin 精确回显，外部管理 Origin 在认证前返回 403，可信响应带 `Vary: Origin`。Cloudflare 版本为 `4633152a-726e-45b2-a3d8-6feba0a07c0f`；生产 13 项 CORS、固定页面、sitemap 43/43、SY300/F29 身份及 F29 五图验证通过，未使用真实管理 Key、发送 POST、读取管理数据或写 D1。详见 `docs/audits/p44-admin-api-cors-boundary.md`。
- P45 已于 2026-08-05 发布：公开询盘 POST 在访问 D1 前强制 `application/json`、32 KB 声明／原始字节上限和 JSON 对象根值；既有字段、5 次／小时限流、蜜罐、SMTP 降级、`emailStatus` 与公开 CORS 均保持不变。Cloudflare 版本为 `8d2d99e4-f935-4618-8244-09784f08d446`；生产固定页面、询盘预检与安全头、sitemap 43/43、SY300/F29 身份及 F29 五图验证通过，未发送生产 POST、使用真实管理 Key 或接触 D1。详见 `docs/audits/p45-inquiry-request-boundary.md`。
...
- P50 已于 2026-08-08 完成并同步：新增 `scripts/verify-production-readonly.mjs`，把固定入口、管理拒绝、真实 404、sitemap、车型身份与 F29 五图固化为只允许同源 GET 的发布后门禁。生产实测共 69 个请求、0 个失败；脚本不接受或读取管理 Key，没有写请求或 D1/SMTP 入口。本阶段无需部署。详见 `docs/audits/p50-production-readonly-verification-script.md`。
...
8. P50 已完成脚本、真实生产只读验证和版本库同步；后续发布统一先完成目标变更，再运行该脚本作为生产只读收口门禁。

## 15. Git 状态摘要

- 当前分支为 `main`；P50 同步完成后应与 `origin/main` 保持一致。
- P50 仅包含新增只读验收脚本、审计报告及 `TASKS.md`、`CODEX_HANDOFF.md` 更新。
- 没有回退、自动清理、修改 `deploy/` 或部署 Cloudflare。
